...
BEGIN_MESSAGE_MAP(CPersonalForm10, CFormView)
	...
	ON_BN_CLICKED(IDC_CMD_NEXT_FORM, &CPersonalForm10::OnBnClickedCmdNextForm)
END_MESSAGE_MAP()
...
void CPersonalForm10::OnBnClickedCmdPrintForm()
{
	...
	ss.str(""); ss.clear();
	ss << "select * from file_form_17 where file_id=" << file_id << " limit 0,14;";
	_vSubformQueryString.push_back(ss.str());
	
	ss.str(""); ss.clear();

	DoPrintForm(CString(_T("表2-9.dotx")));
}
...
void CPersonalForm10::OnBnClickedCmdUpdateForm()
{
	...
	DoUpdateFlag(1, 0, m_Radio12_1_0);
}


void CPersonalForm10::OnBnClickedCmdNextForm()
{
	// TODO:  在此添加控件通知处理程序代码
	CMainFrame* pWnd = (CMainFrame*)AfxGetApp()->m_pMainWnd;
	::PostMessage(pWnd->m_hWnd, WM_CREATE_PERSONAL_FORM, WPARAM(11), LPARAM(new CString(m_strCurrentFolder + _T("/") + m_strCurrentFile)));
}
