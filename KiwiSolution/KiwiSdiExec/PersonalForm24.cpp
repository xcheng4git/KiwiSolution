...
: CFormView(CPersonalForm24::IDD)
{
	...
	vStr.clear(); vStr.push_back(0); vStr.push_back(5); _vvSubformRecordRange.push_back(vStr);

	//以下是为了打印的预设
	const wchar_t *pBookmarks1[3] = { _T("有无"), _T("职务"), _T("工作职责") };
	...
	const wchar_t *pBookmarks2[4] = { _T("有无"), _T("岗位风险点"), _T("风险等级"), _T("防控措施") };
	int structure20[4] = { CBookmarkEx::CHKBOX, CBookmarkEx::TXTBOX, CBookmarkEx::TXTBOX, CBookmarkEx::TXTBOX };
	int structure21[4 + 3] = { -1, 5, 3, 2, 1, 1, 1 };  //有无，行，列，跳过查询结果字段数，每个单元格内的标签数目....
...
void CPersonalForm24::OnBnClickedCmdPrintForm()
{
	...
	ss.str("");  ss.clear();
	ss << "select * from file_form_30_0 where file_id=" << file_id << " limit 0,1;";
	_vSubformQueryString.push_back(ss.str());
	ss.str(""); ss.clear();

	ss.str("");  ss.clear();
	ss << "select * from file_form_30 where file_id=" << file_id << " limit 0,5;";
	_vSubformQueryString.push_back(ss.str());
	ss.str(""); ss.clear();

	DoPrintForm(CString(_T("表9.dotx")));
}
